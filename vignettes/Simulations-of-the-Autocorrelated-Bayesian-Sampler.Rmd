...
-  the fixed stopping rule, `n_sample`;
-  the stimuli of the estimate task, `trial_stim`;

```{r, results = FALSE}
abs_model$simulate(stopping_rule = 'fixed', n_sample = 5, trial_stim = stim)
```
...
abs_model$simulate(stopping_rule = 'fixed', n_sample = 5, trial_stim = stim)$confidence_interval(0.5)
knitr::kable(abs_model$sim_results)
```
...
- the stimuli dependency, `prior_depend`, which controls whether the prior on responses changes based on the previous stimulus. The default setting is `TRUE`, which adjusts the prior by adding 1 according to the previous stimulus.
...
In the upcoming simulations, we'll run the simulation again with two arguments changed: `start_point` and `prior_depend`. To proceed, let us generate some starting points for the simulation.
