...
#### Hamiltonian
HMC needs tuning of the epsilon and L parameters for optimal performance. Compare the performance of HMC in this space depending on the size of epsilon and L (small = red, large = blue)
```{r, dpi=150, fig.align="center"}
# epsilon = .5, L = 10
HMC_.5 <-  sampler_hmc(names, parameters, c(5,5), iterations = iterations, weights = weights)
HMC_.5_df <- data.frame(x = HMC_.5[[1]][,1], y = HMC_.5[[1]][,2])

# epsilon = 1, L = 50
HMC_1 <-  sampler_hmc(names, parameters, c(5,5), L = 50, iterations = iterations, weights = weights, epsilon = 1)
...
hill_map + 
  ...
  geom_path(HMC_1_df, mapping = aes(x,y), colour = "blue", linetype = "dashed", size = .3) + 
  geom_point(HMC_1_df, mapping = aes(x,y), colour = "white",size =.1)
```
...
<!--   geom_path(NUTS_1_df, mapping = aes(x,y), colour = "blue", linetype = "dashed", size = .3) +  -->
