...
1. **Distribution Name**: `"norm"`,
2. **Distribution Parameters**: A vector with the distribution parameters. In our case, `mean` and `std`, that is: `c(0,1)`,
3. A **starting point**: Any point in the sampling space. We'll choose the mean of our distribution, `0`.,
...
MH_samples <- sampler_mcmc(name, params, start, proposal_variance)
```
...
HMC_samples <- sampler_hmc(name, params, start)

NUTS_samples <- sampler_nuts(name, params, start)
```
...
proposal_variance <- 1
MH_samples <- sampler_mcmc(name, params, start, proposal_variance, iterations = 2**12, weights = weights)
# plot
hist(MH_samples[[1]], freq=FALSE, breaks = 20)
